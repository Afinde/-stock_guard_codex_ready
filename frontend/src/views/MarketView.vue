...
async function load() {
  ...
  error.value = ''
  try {
    const [quotePage, status, runPage] = await Promise.all([backend.marketQuotes({ page: 1, page_size: 50 }), backend.providerStatus(), backend.ingestionRuns()])
...
async function runJob(jobType: string) {
  try {
    await backend.runDataJob(jobType)
